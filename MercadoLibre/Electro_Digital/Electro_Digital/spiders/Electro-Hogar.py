...
class MercadoLibreSpider(scrapy.Spider):
    ...
    start_urls = ['https://listado.mercadolibre.com.co/electrodomesticos/refrigeracion/nuevo/',
                  'https://listado.mercadolibre.com.co/pequenos-electrodomesticos/nuevo/',
                  'https://listado.mercadolibre.com.co/lavado/nuevo/',
                  'https://listado.mercadolibre.com.co/electrodomesticos/climatizacion/nuevo/']
    ...
    def parse(self, response):
        categories1 = response.xpath('//dt[contains(text(),"Categorías")]/ancestor::dl')  # Select parent node for label Categoria
        categories1 = categories1.xpath('.//dd')

        for cat in categories1:
            category1_url = cat.xpath('.//a/@href').extract_first()
         
            yield scrapy.Request(category1_url, callback = self.parse_categories)
    ...
    def parse_publicado(self,response):
        ...
        if not nombre:
            ...
            referencia = response.xpath('//th[contains(text(),"Marca")]/following-sibling::*[1]//text()').extract_first()  
